...
RANGES  = os.path.join(ROOT, ".subset_ranges.json")  # what each locale needs, so the
                                                     # CSS can be rebuilt whole
...
def main():
    ...
    # Subsetting is per-locale, but the stylesheet is ONE file rewritten whole.
    # Generating it from `want` alone silently dropped every locale not named on
    # the command line: running `subset_fonts.py ko` un-declared Japanese, whose
    # font file was still on disk, so the build's coverage check saw nothing
    # wrong and ja would have shipped in a system fallback. The CSS is therefore
    # always rebuilt from what actually exists in fonts/.
    faces = []
    ranges = json.load(open(RANGES)) if os.path.exists(RANGES) else {}
    for loc in want:
        ...
        ranges[loc] = (slug, ", ".join(f"U+{c:04X}" for c in cps))
    ...
    json.dump(ranges, open(RANGES, "w"), indent=2)
    for loc in sorted(ranges):
        slug, rng = ranges[loc]
        if not os.path.exists(os.path.join(FONTS, f"{slug}-{loc}.woff2")):
            print(f"  !! {loc}: range recorded but {slug}-{loc}.woff2 is missing")
            continue
